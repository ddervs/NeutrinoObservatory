...
for asdasf = 1:simulits
        ...
        load('100x100x27antarctica.mat') % Matrix of probabilities of interaction locations
        xsec = 5.3E-32;%cm2 interaction cross section
        u = 1.66E-24; %g

        detector_width = 10;
        detector_length = 10;
        detector_depth = 28; % times 100m

        %% Density profile

        den = 300; % Coefficients of density profile
        gam = 0.04;
        AA = 325;
        bet = 1;
        ...
        rho = avRefIndex(0,100*detector_depth,denscoeff); %gcm-3
...
display(['Expected number of interactions that take place/yr: ', num2str(mean(numofintstakenplace)), ' +- ',...
    num2str((std(numofintstakenplace)*rootN))])

hist(numofobservedints, 0:round(max(numofobservedints)))
xlabel('Number of observed interactions')
ylabel('Frequency')
title(['Antarctica: Results of Monte-Carlo simulation, # of iterations: ', num2str(simulits) ])
